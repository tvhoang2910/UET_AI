...
class ChatServiceTest {
        ...
        @Mock
        private vn.edu.uet.chatbot.cache.SemanticCacheService semanticCacheService;

        @Mock
        private vn.edu.uet.chatbot.router.QueryCategoryRouter categoryRouter;

        @Mock
        private vn.edu.uet.chatbot.service.PrerequisiteTracker prerequisiteTracker;
        ...
        @BeforeEach
        void setUp() {
                lenient().when(semanticCacheService.findSimilar(anyList())).thenReturn(Optional.empty());
                lenient().when(categoryRouter.detectCategory(anyString())).thenReturn(Optional.empty());
                lenient().when(prerequisiteTracker.extractPrerequisiteChainSummary(anyString(), anyList()))
                                .thenReturn("");
        }
        ...
        void should_return_no_info_without_calling_llm_when_no_chunks_are_found() {
                ...
                when(embeddingModel.embed("Mỗi bài học được chia thành các phần như thế nào?"))
        ...
        void should_include_source_context_in_prompt_when_chunks_are_found() {
                ...
                assertThat(promptCaptor.getValue())
                                .contains("--- [NGUỒN 1 | TÀI LIỆU: \"Tiếng Việt Dễ Dàng\" | TRANG: 2] ---");
        ...
        @Test
        void should_clear_security_context_when_stream_subscribe_fails_synchronously() {
                ...
                when(messageRepository.findTop10BySessionIdOrderByCreatedAtDesc(sessionId)).thenReturn(List.of());
                when(messageRepository.save(org.mockito.ArgumentMatchers.any(ChatMessageEntity.class)))
                ...
                when(vectorStore.searchWithScores(anyList(), anyInt(), anyString(), anyDouble(),
                                nullable(String.class)))
                                .thenReturn(List.of(new ScoredDocumentChunk(
                                                new DocumentChunk("doc-1", 0, 1, "Đoạn đạt ngưỡng",
                                                                Map.of("title", "Doc One")),
                                                0.91)));
                when(chatModel.stream(org.mockito.ArgumentMatchers.any(Prompt.class))).thenReturn(Flux.create(sink -> {
                        throw new IllegalStateException("subscribe fail");
                }));

                SecurityContextHolder.getContext().setAuthentication(
                                new TestingAuthenticationToken(USERNAME, "pw", "ROLE_USER"));

                SseEmitter emitter = chatService.chatStream(sessionId, "Câu hỏi bất kỳ", USERNAME);
